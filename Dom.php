...
/**
 * SimpleHtmlDom wrapper built for symfony
 * Version 0.1
 * @author Yiqiang <yiqiang.dai@gmail.com>
 */
namespace YQ\GlobalBundle\Helper;

// helper functions
// -----------------------------------------------------------------------------
// get html dom from file
// $maxlen is defined in the code as PHP_STREAM_COPY_ALL which is defined as -1.
class Dom extends Dom_Constant
{
    function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = self::DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=self::DEFAULT_BR_TEXT, $defaultSpanText=self::DEFAULT_SPAN_TEXT)
    {
        // We DO force the tags to be terminated.
        $dom = new dom_parse(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    ...
    function str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = self::DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=self::DEFAULT_BR_TEXT, $defaultSpanText=self::DEFAULT_SPAN_TEXT)
    {
        $dom = new dom_parse(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
class Dom_Node extends Dom_Constant
{
		...
		if (isset($this->attr['style']))
		{
			...
			if (isset($attributes['width']) && $width == -1)
			{
				...
				if (strtolower(substr($attributes['width'], -2)) == 'px')
				{
					...
					if (filter_var($proposed_width, self::FILTER_VALIDATE_INT))
					{
			...
			if (isset($attributes['height']) && $height == -1)
			{
				...
				if (strtolower(substr($attributes['height'], -2)) == 'px')
				{
					...
					// Now make sure that it's an integer and not something stupid.
					if (filter_var($proposed_height, self::FILTER_VALIDATE_INT))
					{
 ...
 */
class dom_parse extends Dom_Constant
{
	public $root = null;
	public $nodes = array();
	...
	function save($filepath='')
	{
		...
		if ($filepath!=='') file_put_contents($filepath, $ret, self::LOCK_EX);
		return $ret;
	}
	...
	function is_exist($selector, $idx=null, $lowercase=false)
	{
		$o=$this->find($selector,$idx,$lowercase);
		if(is_object($o))
		{
			return true;
		} else {
